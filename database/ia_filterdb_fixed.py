...
async def save_file(media):
    ...
    file_type=media.mime_type.split('/')[0] if getattr(media, 'mime_type', None) else None,
    message_id=getattr(media, 'id', None),
    thumb=getattr(getattr(media, 'thumbs', [None])[0], 'file_id', None) if getattr(media, 'thumbs', None) else getattr(getattr(media, 'thumb', None), 'file_id', None)
)
